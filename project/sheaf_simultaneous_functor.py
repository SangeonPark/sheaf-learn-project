...
class sheaf_gradient_flow_functor(pl.LightningModule):
    ...
    def __init__(self, graph, Nv, dv, Ne, de, adjacency, max_deg, layers, input_dim, output_dim, channels, left_weights, right_weights, potential, mask, use_act, augmented, add_lp, add_hp, dropout, input_dropout, learning_rate = 0.01):
        ...
        self.graph_to_sheaf = nn.Linear(input_dim+self.Nv, self.final_d*channels+max_deg*dv*de)
        self.graph_to_sheaf2  = nn.Linear(self.final_d*channels+max_deg*dv*de, self.final_d*channels+max_deg*dv*de)
        self.graph_to_sheaf3  = nn.Linear(self.final_d*channels+max_deg*dv*de, self.final_d*channels+max_deg*dv*de)
    ...
    def _common_step(self, batch, batch_idx, stage: str):
        ...
        #print(batch.size())
        self.adjacency = self.adjacency.view(-1, self.Nv, self.Nv).to(batch)
        batch = torch.cat((batch, self.adjacency), dim=2)
        #print("append adj:", batch.size())
        ...
        batch_vertex     = batch[:, :, :self.hidden_dim]
        #print("batch vertex:", batch_vertex.size())
        batch_coboundary = batch[:, :, self.hidden_dim:]
        ...
        batch_coboundary = batch_coboundary.reshape(-1, self.max_deg, self.de, self.dv)
        #print("batch vertex:", batch_vertex.size())
        #print("batch coboundary:", batch_coboundary.size())
      ...
      #  #if self.second_linear:
        ...
        for layer in range(self.layers):
            ...
            for i, (v1, v2) in enumerate(self.graph.edges):
                coboundary[self.de*i:self.de*(i+1),self.dv*v1:self.dv*(v1+1)] = 1. * batch_coboundary[v1, list(self.graph.neighbors(v1)).index(v2), :, :]
                coboundary[self.de*i:self.de*(i+1),self.dv*v2:self.dv*(v2+1)] = -1.* batch_coboundary[v2, list(self.graph.neighbors(v2)).index(v1), :, :]

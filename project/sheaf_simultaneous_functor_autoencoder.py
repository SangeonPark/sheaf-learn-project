...
class sheaf_gradient_flow_functor(pl.LightningModule):
    ...
    def __init__(self, graph, Nv, dv, Ne, de, adjacency, max_deg, layers, input_dim, output_dim, channels, left_weights, right_weights, potential, mask, use_act, augmented, add_lp, add_hp, dropout, input_dropout, learning_rate = 0.01):
        ...
        self.graph_to_sheaf = nn.Linear(input_dim * 2, 1500)
        self.graph_to_sheaf2  = nn.Linear(1500, 1500)
        #self.graph_to_sheaf3  = nn.Linear(300, 300)
        #self.graph_to_sheaf4  = nn.Linear(1000, 1000)
        self.graph_to_sheaf3  = nn.Linear(1500, 2*(self.final_d*channels)+2*dv*de)
    ...
    def _common_step(self, batch, batch_idx, stage: str):
        ...
        batch = self.initial1(batch)
        batch = F.elu(batch)
        batch = F.dropout(batch, p=self.dropout, training=training)
        batch = self.initial2(batch)
        batch = F.elu(batch)
        batch = F.dropout(batch, p=self.dropout, training=training)
        batch = self.initial3(batch)
      ...
      #  #if self.second_linear:
        ...
        for layer in range(self.layers):
            ...
            for i, (v1, v2) in enumerate(self.graph.edges):
                coboundary[self.de*i:self.de*(i+1),self.dv*v1:self.dv*(v1+1)] = 1. * batch_coboundary[i,0,:,:]
                coboundary[self.de*i:self.de*(i+1),self.dv*v2:self.dv*(v2+1)] = -1.* batch_coboundary[i,1,:,:]

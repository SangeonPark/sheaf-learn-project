...
#sweep_id = wandb.sweep(sweep_config, project="Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold0")
#/Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold_all/sweeps/zvf0s60g
sweep_id = 'saepark/Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold_all/284rnquo'
print(sweep_id)
...
def sweep_iteration_2():
    ...
    wandb_logger_diffusion = WandbLogger(project="Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold_all")
    early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=0.000, patience=30, verbose=False, mode='min')
    path = f'/root/projects/temp_checkpoints/Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold_all/wisconsin_{wandb.config.fold}/dv{wandb.config.dv}-de{wandb.config.dv}-layers{wandb.config.layers}-channels{wandb.config.channels}-firsthidden{wandb.config.first_hidden}-secondhidden{wandb.config.second_hidden}'
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_acc",
        dirpath=path,
        filename="functor_onlyxflow_radial_dec26-{epoch:02d}-{val_loss:.2f}-{val_acc:.4f}",
        save_top_k=50,
        mode="max",
    )
    data = get_fixed_splits(data_source, 'wisconsin', wandb.config.fold)
    ...
    mypath = f'/root/projects/temp_checkpoints/Sweep_functor_onlyxflow_radial_Dec_26_wisconsin_fold_all/wisconsin_{wandb.config.fold}/dv{wandb.config.dv}-de{wandb.config.dv}-layers{wandb.config.layers}-channels{wandb.config.channels}-firsthidden{wandb.config.first_hidden}-secondhidden{wandb.config.second_hidden}'
...
wandb.agent(sweep_id, function=sweep_iteration_2)
